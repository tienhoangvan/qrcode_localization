...
def qrcode_localization():
	...
	pub_camera_pose = rospy.Publisher('camera_pose', PoseWithCovarianceStamped, queue_size=10)
	...
	while True:
		...
		for qrcode in qrcodes:
			...
			camera_pose = PoseWithCovarianceStamped()
			quat = Quaternion()

			camera_pose.header.frame_id = "camera_logi"
    			camera_pose.header.stamp = rospy.Time.now()

			camera_pose.pose.pose.position.x = camera_pose_x
			camera_pose.pose.pose.position.y = camera_pose_y
			
			quat = quaternion_from_euler(0.0, 0.0, camera_pose_theta)
			camera_pose.pose.pose.orientation = Quaternion(*quat)
	
			pub_camera_pose.publish(camera_pose)
			rospy.loginfo("Camera Pose:" + str(camera_pose))
